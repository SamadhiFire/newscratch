...
| `新闻正文` | text | Generated body, 600-800 Chinese chars |
...
| `优化后正文` | text | Final generated body |
...
If `publishedAt` is missing, use the run time. If `status` is missing, use `已生成`. If `publishStatus` is missing, use `未发布`.
